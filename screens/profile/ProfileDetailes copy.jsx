...
import { FontAwesome, Feather, Fontisto, SimpleLineIcons, Ionicons, AntDesign } from '@expo/vector-icons'
    ...
    <View style={{ flex: 1, backgroundColor: theme ? color.dark : color.white, paddingTop: 10 }}>
...

      {
        ...
        <View style={_profile.infoListContainer}>
          ...
          <View style={_profile.infoList}>
            <OymoFont message=' in' fontStyle={{ ..._profile.title, color: theme ? color.white : color.dark }} />
            <OymoFont message={`${profile?.address?.city}, ${profile?.address?.country}`} fontStyle={{ ..._profile.info, color: theme ? color.white : color.dark }} fontFamily='montserrat_bold' />
          </View>
        </View>
      }
