    ...
    <>
      {
        events?.length < 1 ?
          ...
          <FlatList
            ...
            style={pEvents.container}
            onEndReached={() => {
              setLimit(eventsLimit + 4)
              getEvents()
            }}
            ListFooterComponent={() => <View style={pEvents.listFooterComponent} />}
            renderItem={({ item: event }) => (
              <Pressable
                onPress={() => navigation.navigate('Event', { event })}
                // onLongPress={() => navigation.navigate('ReelsOption', { event })}
                delayLongPress={500}
